...
public class BinaryManager extends JFrame {
    ...
    private void init() {
        ...
        Cron.interval(5, SECONDS, new CronJob() {
            ...
            public void exec(final CronStatus status) throws Throwable {
                lock.lock();
                try {
                    final long[] pids = ProcessQueryFactory.getInstance().getQuery(
                            new File(BinaryManager.this.yaeb.getText()).isDirectory()
                                    ? "Exe.Name.ew=mdm_flash_player"
                                    : "Args.*.ew=profile").find(sigar);
    ...
    private void start(final int delay, final Bot bot) {
        ...
        if (yaeb.isDirectory()) {
            ...
            final File base = yaeb.getParentFile();
            final File sub = new File(base, profileApp.getName());
            if (!sub.exists())
                sub.mkdir();
            final File subBot = new File(sub, yaeb.getName());
            try {
                if (!subBot.exists())
                    copyFolder(yaeb, sub);
            } catch (IOException e) {
            ...
            builder.directory(sub);
            builder.command(subBot.getAbsolutePath(), "-autostart", "-autorun", "-profile",
                    bot.profile, "-server", bot.server, "-delay", Integer.toString(delay));
        }
        try {
            ...
            if (yaeb.isDirectory() && !profileApp.renameTo(temp))
